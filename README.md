...
- **语言模型驱动**，当前发布的版本经过了百亿字数语料训练
- **多平台应用**，当前支持 Linux、Android 与 macOS（TODO: iOS）
...
下载 `Sime-*.pkg` 双击安装（需要管理员密码）。安装后到「系统设置 → 键盘 → 输入法」中添加「是语输入法」。

首次打开如被 Gatekeeper 拦截，右键点击 .pkg 选「打开」即可。
