...
使用发布版模型、Top 10 重排和单线程 CPU 推理，对同一批输入重复测量的端到端
解码延迟如下。这里的 `baseline` 为未启用 GRU 的 Sime，`GRU` 为启用后结果；数值
反映本机相对开销，而非不同设备之间可直接比较的绝对性能。

| 模式 | 基线 median / p95 | GRU median / p95 | p95 变化 |
|---|---:|---:|---:|
| 全拼 | 2.800 / 2.822 ms | 2.797 / 2.830 ms | +0.007 ms（+0.26%） |
| T9 | 7.456 / 7.472 ms | 7.474 / 7.499 ms | +0.026 ms（+0.35%） |

因此，GRU 的 8.64 MiB 附加模型空间换来了六集合整体 +5.07 个百分点的句准确率，
同时没有可感知的端到端延迟回退。模型加载仍发生在 Sime 初始化时；每次解码只对
已有的前 10 个候选执行重排。
